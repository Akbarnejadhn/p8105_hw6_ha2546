...
We can observe randomness which which shows homoscedasticity (constant variance) in residuals. This indicates that my model is correctly specified and there is no corrolation between residuals and fitted values. So, there is no need for transformations in my outcome or predictor, and that the linear model is working well, so, we do not need to consider higher order regression models. 
