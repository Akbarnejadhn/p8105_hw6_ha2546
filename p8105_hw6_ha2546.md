...
for each bootstrap sample, produce estimates of \(\hat{r}^2\) and
\(log(\hat{\beta_0} * \hat{\beta_1})\) quantities. first, we should
extract values of \(\hat{\beta_0}\), \(\hat{\beta_1}\), and
...
\(log(\hat{\beta_0} * \hat{\beta_1})\) and (0.9056913 , 0.9171143) for
\(\hat{r}^2\).
